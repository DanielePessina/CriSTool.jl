...
@testset "Kinetics" begin
    ...
    @testset "Nucleation Rate - nucl_CNT" begin
        ...
        expected_super = exp(38.0) * 1.5 *
                         exp(-16π * 0.0007^3 * problem.molecular_volume^2 /
                             (3 * (problem.boltzmann_constant * temperature)^3 * log(1.5)^2))
    ...
    @testset "Nucleation Rate - nucl_empirical" begin
        ...
        rate_super = CriSTool.nucleationrate(nucl_empirical(), params, problem,
                                             state_at_S(problem, 1.5), t)
        @test rate_super ≈ 10.0^10.0 * 0.5^2.0
    ...
    @testset "CNT without leading supersaturation factor" begin
        problem = make_test_problem()
        parameters = [38.0, 0.0007]
        state = state_at_S(problem, 1.5)
        cnt_rate = CriSTool.nucleationrate(nucl_CNT(), parameters, problem, state, 0.0)
        no_s_rate = CriSTool.nucleationrate(nucl_CNTnoS(), parameters,
                                            problem, state, 0.0)
        @test no_s_rate ≈ cnt_rate / 1.5
    end
    ...
    @testset "Growth Rate - growth_empirical" begin
        ...
        rate_super = CriSTool.growthrate(growth_empirical(), params, problem,
                                         state_at_S(problem, 1.5), t)
        @test rate_super ≈ (1e-9 / 60) * 0.5^3.0
    ...
    @testset "Growth Rate - growth_BCF" begin
        ...
        rate_super = CriSTool.growthrate(growth_BCF(), params, problem,
                                         state_at_S(problem, 1.5), t)
        temperature = CriSTool.temperature(problem.temp_profile, t)
        expected_super = (1e-9 / 60) * temperature / 1000.0 * 0.5 *
    ...
    @testset "Nucleation Rate Type Stability" begin
        ...
        @test result ≈ exp(38.0) * 1.5 *
                       exp(-16π * 0.0007^3 * problem.molecular_volume^2 /
                           (3 * (problem.boltzmann_constant * CriSTool.temperature(problem.temp_profile, 0.0))^3 *
                            log(1.5)^2))
    end
    ...
    @testset "Migrated kinetic families use problem state" begin
        ...
        expected_cnt = exp(38.0) * supersaturation_value *
                       exp(-16π * 0.0007^3 * problem.molecular_volume^2 /
                           (3 * (problem.boltzmann_constant * temperature)^3 * log(supersaturation_value)^2))
        @test CriSTool.nucleationrate(CriSTool.nucl_CNT_fixed([38.0, 0.0007]),
                                      Float64[], problem, state, 0.0) ≈ expected_cnt

        expected_empirical = 10.0^10.0 * (supersaturation_value - 1.0)^2.0
        @test CriSTool.nucleationrate(CriSTool.nucl_empirical_fixed([10.0, 2.0]),
                                      Float64[], problem, state, 0.0) ≈ expected_empirical

        expected_growth = (1e-9 / 60) * (supersaturation_value - 1.0)^3.0
        @test CriSTool.growthrate(CriSTool.growth_empirical_fixed([1e-9 / 60, 3.0]),
    ...
    @testset "Composite kinetics dispatch" begin
        ...
        composite_params = [8.0, 2000.0, 6.0, 1.0, 2000.0, 1.5]
        expected_composite =
    ...
    @testset "Dissolution uses the configured saturation model" begin
        ...
        parameters = [2e-9 / 60, 4000.0, 1.5]
        temperature = CriSTool.temperature(problem.temp_profile, 0.0)
        expected_scalar = -(2e-9 / 60) *
                          exp(-4000.0 / (problem.R_gas_constant * temperature)) *
                          (1.0 - 0.5)^1.5
    ...
    @testset "Combined growth and dissolution dispatch" begin
        ...
        combined_parameters = [1.0 - log10(60), 3.0,
                               2e-9 / 60, 4000.0, 1.5]
    ...
    @testset "Breakage rates dispatch on the configured mesh" begin
        ...
        empirical = CriSTool.breakagerate(CriSTool.breakage_empirical(),
                                           [0.5 / 60, 1.0], problem, state, 0.0)
        @test length(empirical) == solver.meshsize
        empirical_parent_length = solver.cell_centre[end]
        empirical_parent_frequency = (0.5 / 60) * empirical_parent_length^3
        ...
        uniform = CriSTool.breakagerate(CriSTool.breakage_uniform(),
                                         [log(0.5 / 60), 1.0], problem, state, 0.0)
        @test length(uniform) == solver.meshsize
        uniform_parent_length = solver.cell_centre[end]
        uniform_parent_frequency = (0.5 / 60) * (uniform_parent_length / 1e-6)^3
