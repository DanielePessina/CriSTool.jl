...
function main()
    ...
    # Flat parameter vector: [ln_nucleation_prefactor, surface_energy,
    ...
    cool_T = CriSTool.CallableTemperature(t -> t < 7200.0 ? 297.0 :
                                              t < 18000.0 ? 297.0 + (290.0 - 297.0) * (t - 7200.0) / 10800.0 :
                                              290.0)
    ...
    lines!(ax_T, sol_const.time ./ 60, [CriSTool.temperature(const_T, t) for t in sol_const.time],
           color = :dodgerblue, label = "constant")
    lines!(ax_T, sol_cool.time ./ 60,  [CriSTool.temperature(cool_T,  t) for t in sol_cool.time],
           color = :crimson,    label = "cooling ramp")
    lines!(ax_T, sol_sweep.time ./ 60, [CriSTool.temperature(sweep_T, t) for t in sol_sweep.time],
           color = :seagreen,   label = "sinusoidal")
    axislegend(ax_T; position = :rb)

    ax_c = CairoMakie.Axis(fig[2, 1], xlabel = "Time (min)", ylabel = "c (mg/mL)")
    lines!(ax_c, sol_const.time ./ 60, sol_const.concentration, color = :dodgerblue)
    lines!(ax_c, sol_cool.time ./ 60,  sol_cool.concentration,  color = :crimson)
    lines!(ax_c, sol_sweep.time ./ 60, sol_sweep.concentration, color = :seagreen)

    ax_d = CairoMakie.Axis(fig[3, 1], xlabel = "Time (min)", ylabel = "d43 (μm)")
    lines!(ax_d, sol_const.time ./ 60, 1e6 .* sol_const.d43, color = :dodgerblue)
    lines!(ax_d, sol_cool.time ./ 60,  1e6 .* sol_cool.d43,  color = :crimson)
    lines!(ax_d, sol_sweep.time ./ 60, 1e6 .* sol_sweep.d43, color = :seagreen)
